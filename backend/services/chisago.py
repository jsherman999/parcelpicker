...
class ChisagoParcelService(BaseParcelService):

    endpoint_url = CHISAGO_QUERY_URL
    ...
    def _street_line(self, address: str) -> str | None:
        ...
        # PropAddr is street-only (with trailing spaces), so drop a trailing
    ...
    def _build_address(self, attrs: dict[str, Any]) -> str:
        ...
        city = str(attrs.get("PropCity") or "").strip()
        zip_val = str(attrs.get("PropZip") or "").strip()
