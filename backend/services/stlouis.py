...
ST_LOUIS_QUERY_URL = (
    ...
    "GeneralUse/Open_Data/MapServer/7/query"
)
CENSUS_GEOCODE_URL = "https://geocoding.geo.census.gov/geocoder/locations/onelineaddress"

# Fields available in the St. Louis County parcel layer
# Parcel ID: PRCL_NBR (255-char string)
# Owner: OWNAME
# Address: PHYSADDR + PHYSCITY + PHYSZIP
ST_LOUIS_OUT_FIELDS = "PRCL_NBR,OWNAME,PHYSADDR,PHYSCITY,PHYSZIP"


class StLouisParcelProvider(ParcelProvider):
    def __init__(
        self,
        *,
    ...
    async def geocode_address(
        ...
        *,
        ...
        # St. Louis County's built-in geocoder returns 0 results,
        # so Census is the primary path.
    ...
    async def lookup(
        ...
        *,
        ...
        if feature is not None:
            record = self._feature_to_record(feature, matched_by="stlouis_address")
        ...
        if street_only and street_only != cleaned:
            ...
            if feature is not None:
                record = self._feature_to_record(
                    feature,
                    matched_by="stlouis_address_from_census",
                )
                self._cache_record(cleaned, record)
    ...
    async def query_adjacent(
        ...
        *,
        ...
        features = await self._query_st_louis(
            {
                "where": "1=1",
        ...
        for feature in features:
            record = self._feature_to_record(feature, matched_by="stlouis_touches")
    ...
    async def _query_by_address(
        ...
        *,
        ...
        # St. Louis PHYSADDR stores the full site address as a single string
        # (e.g., "4706 OAKLEY ST"). Try exact match first, then contains.
    ...
    async def _query_st_louis(
        ...
        *,
        ...
        data = await self._get_json(ST_LOUIS_QUERY_URL, payload, budget=budget)
        if "error" in data:
            message = data["error"].get("message", "ArcGIS query error")
            raise RuntimeError(f"St. Louis County parcel query failed: {message}")
    ...
    def _feature_to_record(
        ...
        *,
        ...
        parcel_id = str(attrs.get("PRCL_NBR") or "").strip()
        owner_name = str(attrs.get("OWNAME") or "").strip()

        # PHYSADDR is the full site address, PHYSCITY is the city
        physaddr = str(attrs.get("PHYSADDR") or "").strip()
        physcity = str(attrs.get("PHYSCITY") or "").strip()

        if physaddr and physcity:
            site_address = f"{physaddr}, {physcity}"
        elif physaddr:
            site_address = physaddr
        elif physcity:
            site_address = physcity
        else:
            site_address = ""
